      ...
      <div class="container">
          ...
          <div class="content">
              ...
              <div v-show="findEndPoint" >
                <ul id="searchResult" name="searchResult">
                  <div v-for="(item, index) in searchResult" :key="index">
                    ...
                    {{endIndex}}
                    <img :src="item.src" style='vertical-align:middle;'/><span>{{item.name}}</span>
                    <b-button variant='warning' name='sendBtn' @click="selected(item,1,index)">도착점 설정</b-button>
                  </div>
                </ul>
              </div>
            ...
            <div class="itemList">
                ...
                <b-button @click="findRouteTmap()" :disabled="Object.keys(startPointObject).length0==0 || Object.keys(endPointObject).length==0">검색</b-button>
                <hr>
                  <div v-for="(item,index) in stopOverList" :key="index" style="display:flex; justify-content:space-between;">
                    <i class="fi fi-sr-Cross-circle"></i>
                    <span style="font:25px;">
                      {{item.name}}
                    </span>
                    <img src="../../static/icon/cross.png" style="width:20px; height:20px;" @click="deleteStopOver(index)">
                  </div>
                <hr>
    ...
    methods:{
      pickStopOver(){
        ...
        for(let i=0; i<this.stopOverList.length;i++){
          let mapInstance = new Tmapv2.Marker({
            position : new Tmapv2.LatLng(this.stopOverList[i].lat, this.stopOverList[i].lon),
            icon : "http://tmapapi.sktelecom.com/upload/tmap/marker/pin_g_m_"+i+".png",
      ...
      findRouteTmap(){
        ...
        for(let i in this.stopOverList){
          let via={
            "viaPointId": "test"+i,
        ...
        $.ajax({
          ...
          success:function(response){
            ...
            // 결과 출력
            let tDistance = "총 거리 : " + (resultData.totalDistance/1000).toFixed(1) + "km,  ";
            let tTime = "총 시간 : " + (resultData.totalTime/60).toFixed(0) + "분";
            ...
            for(let i in resultFeatures) {
              ...
              for(let i in this.stopOverObjectList){
                this.resultInfoArr.push(this.stopOverObjectList[i])
              }
              ...
              if(geometry.type == "LineString") {
                ...
                let size = "";			//아이콘 크기 설정합니다.
										...
										let latlon = new Tmapv2.Point(geometry.coordinates[0], geometry.coordinates[1]);
